...
\par Le problème SBLS (ou Carré Latin Spatialement Équilibré) possède une contrainte supplémentaire comparé au problème du Carré Latin. Cette contrainte supplémentaire contraint que la somme des distances entre chaque paire de nombres \( i \) et \( j \) est égale, \( \forall i \in [0, n-1] \), \( \forall j \in [0, n-1] \).
...
\par \href{https://www.oracle.com/fr/java/technologies/downloads/#java21}{Java version 21.0.8 distribué par Oracle} ainsi que \href{https://github.com/chocoteam/choco-solver/releases/tag/v4.10.14}{Choco-solver version 4.10.14} ont été utilisés.
...
\par Pour pouvoir implémenter et résoudre le problème SBLS avec Choco-solver, on a choisit de modéliser le problème SBLS avec les variables, les domaines et les contraintes suivantes.
...
\par \(Variables\) est définit mathématiquement de la manière suivante :
\[
...
\par Dans notre programme \( Variables \) est définit de la manière suivante :
\begin{verbatim}
...
\par Les domaines des variables est définit mathématiquement de la manière suivante:
\[
...
\\ 
\par Cela signifie que tout élément \( elt \in [0, n-1] \) peut prendre pour position entre \(0\) et \(n-1\) au sein de chaque ligne \( i \in [0, n-1] \) du carré \( n \times n \).

\subsection{Contraintes}

\subsubsection{Contrainte sur les lignes}

\par La première contrainte du problème SBLS proscrit que il y a exactement une seule occurence de chaque élément \( elt \) au sein d'une ligne \( i \) du carré \(n \times n \) \( \forall i \in [0, n-1] \).
\par Dans notre modélisation, comme la variable \(v_{i, elt}\) représente la position de l'élément \( elt \) au sein de la ligne i \( \forall v_{i, elt} \in Variables \), alors chaque ligne est déjà implicitement contraint de contenir tout élément un seule fois. En effet un élément \( elt \) ne peut être à deux positions différentes, comme tout \(v_{i, elt} \in Variables \) ne peut que prendre une seule valeur parmi son domaine \( D_{i, elt} = \{0, \dots , n-1\} \).
\par Cependant il est nécéssaire de contraindre notre modèle tel que deux éléments \( elt \) distincts ne peuvent être à la même position, comme toute case du carré \( n \times n \) ne peut contenir que un seul élément.
\par Mathématiquement cette contrainte est définit de la manière suivante :
\[
...
\par Dans notre programme cette contrainte à été implémentée de la manière suivante :
\begin{verbatim}
for (int i = 0; i < n; i++) {
