...
\[
Variables = \{ v_{i, elt}, \forall i \in [0 \dots n-1], \forall elt \in [0 \dots n-1] \}
\]
où \( v_{i, elt} \) représente la position de l'élément \( elt \in [0 \dots n-1] \) au sein de la ligne \( i \in [0 \dots n-1] \) du carré \( n \times n \).

\subsection{Domaines}

\par Les domaines des variables est définit mathématiquement de la manière suivante:
\[
Domaines = \{ D_{i, elt}, \forall i \in [0 \dots n-1], \forall elt \in [0 \dots n-1] \}
\]
avec
\[
D_{i, elt} = \{0, \dots, n-1\}, \forall i \in [0 \dots n-1], \forall elt \in [0 \dots n-1]
\]
où \( D_{i, elt} \) est le domaine de la variable \( v_{i, elt} \), \( \forall i \in [0 \dots n-1], \forall elt \in [0 \dots n-1] \)
\\ 
\par Cela signifie que tout élément \( elt \in [0 \dots n-1] \) peut prendre pour position entre \(0\) et \(n-1\) au sein de chaque ligne \( i \in [0 \dots n-1] \) du carré \( n \times n \).
